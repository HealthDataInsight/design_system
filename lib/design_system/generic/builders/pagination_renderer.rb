      ...
      class PaginationRenderer < WillPaginate::ActionView::LinkRenderer
        ...
        def previous_or_next_page(page, text, classname)
          if classname.include?('previous_page')
            build_page_item(page, text, 'Previous', previous_icon, "#{brand}-pagination-item--previous")
          elsif classname.include?('next_page')
            build_page_item(page, text, 'Next', next_icon, "#{brand}-pagination-item--next")
          end
        end

        private

        def build_page_item(page, text, title, icon, css_class)
          if page
            tag(:li, link_with_title(text, page, title, icon), class: css_class)
          else
            tag(:li, disabled_link_with_title(text, title), class: css_class)
          end
        end
