class CharacterCount {
  constructor($module) {
    this.$module = $module
    this.$textarea = $module.querySelector('.ndrsuk-js-character-count')
  ...
  init() {
    ...
    $screenReaderCountMessage.className = 'ndrsuk-character-count__sr-status ndrsuk-u-visually-hidden'
    ...
    $visibleCountMessage.classList.add('ndrsuk-character-count__status')
    $visibleCountMessage.setAttribute('aria-hidden', 'true')
    this.$visibleCountMessage = $visibleCountMessage
    $fallbackLimitMessage.insertAdjacentElement('afterend', $visibleCountMessage)

    // Hide the fallback limit message
    $fallbackLimitMessage.classList.add('ndrsuk-u-visually-hidden')
  ...
  updateVisibleCountMessage() {
    ...
    if (this.isOverThreshold()) {
      $visibleCountMessage.classList.remove('ndrsuk-character-count__message--disabled')
    } else {
      $visibleCountMessage.classList.add('ndrsuk-character-count__message--disabled')
    }

    // Update styles
    if (remainingNumber < 0) {
      $textarea.classList.add('ndrsuk-textarea--error')
      $visibleCountMessage.classList.remove('ndrsuk-hint')
      $visibleCountMessage.classList.add('ndrsuk-error-message')
    } else {
      $textarea.classList.remove('ndrsuk-textarea--error')
      $visibleCountMessage.classList.remove('ndrsuk-error-message')
      $visibleCountMessage.classList.add('ndrsuk-hint')
    }
...
export default ({ scope = document } = {}) => {
  const characterCounts = scope.querySelectorAll('[data-module="ndrsuk-character-count"]')
