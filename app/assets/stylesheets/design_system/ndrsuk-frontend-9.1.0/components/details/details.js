...
export default ({ scope = document } = {}) => {
  ...
  const initDetails = (element, index) => {
    ...
    element.setAttribute('ndrsuk-polyfilled', 'true')

    // Give details element an ID if it doesn't already have one
    if (!element.id) element.setAttribute('id', `ndrsuk-details${index}`)

    // Set content element and give it an ID if it doesn't already have one
    const content = scope.querySelector(`#${element.id} .ndrsuk-details__text`)
    if (!content.id) content.setAttribute('id', `ndrsuk-details__text${index}`)

    // Set summary element
    const summary = scope.querySelector(`#${element.id} .ndrsuk-details__summary`)
  ...
  if (allDetails.length) {
    allDetails.forEach((element, index) => {
      if (!element.hasAttribute('ndrsuk-polyfilled')) initDetails(element, index)
    })
  }
}
