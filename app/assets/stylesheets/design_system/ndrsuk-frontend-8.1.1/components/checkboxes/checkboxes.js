...
const syncAllConditionalReveals = function syncAllConditionalReveals(input) {
  ...
  allInputsInForm.forEach((item) => toggleConditionalInput(item, 'ndrsuk-checkboxes__conditional--hidden'))
}
...
export default ({ scope = document } = {}) => {
  ...
  const checkboxInputs = scope.querySelectorAll('.ndrsuk-checkboxes .ndrsuk-checkboxes__input')
  ...
  const handleClick = (event) => {
    ...
    toggleConditionalInput(event.target, 'ndrsuk-checkboxes__conditional--hidden')
