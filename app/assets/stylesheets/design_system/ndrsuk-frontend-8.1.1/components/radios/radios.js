...
export default ({ scope = document } = {}) => {
  ...
  const radioInputs = scope.querySelectorAll('.ndrsuk-radios--conditional .ndrsuk-radios__input')

  /**
   * Update all conditional reveals to match checked state
   */
  const syncAllConditionalReveals = () => {
    radioInputs.forEach((input) => toggleConditionalInput(input, 'ndrsuk-radios__conditional--hidden'))
  }
